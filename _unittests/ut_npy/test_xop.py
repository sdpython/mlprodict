...
class TestXOps(ExtTestCase):
    ...
    def test_onnx_abs_1(self):
        OnnxAbs, OnnxIdentity = loadop("Abs", "Identity")

        with self.subTest(op="+"):
            ab = OnnxAbs('X')
            ov = OnnxIdentity(ab + 1, output_names=['Y'])
            onx = ov.to_onnx(numpy.float32, numpy.float32, verbose=0)
            self.assertNotIn("elem_type: 0", str(onx))
            oinf = OnnxInference(onx)
            x = numpy.array([-2, 2], dtype=numpy.float32)
            got = oinf.run({'X': x})
            self.assertEqualArray(numpy.abs(x) + 1, got['Y'])

        with self.subTest(op="-"):
            ov = OnnxIdentity(OnnxAbs('X') - 1, output_names=['Y'])
            onx = ov.to_onnx(numpy.float32, numpy.float32, verbose=0)
            self.assertNotIn("elem_type: 0", str(onx))
            oinf = OnnxInference(onx)
            x = numpy.array([-2, 2], dtype=numpy.float32)
        ...
        with self.subTest(op="*"):
            ov = OnnxIdentity(OnnxAbs('X') * 2, output_names=['Y'])
            onx = ov.to_onnx(numpy.float32, numpy.float32, verbose=0)
            self.assertNotIn("elem_type: 0", str(onx))
            oinf = OnnxInference(onx)
            x = numpy.array([-2, 2], dtype=numpy.float32)
        ...
        with self.subTest(op="=="):
            ov = OnnxIdentity(OnnxAbs('X') == 2, output_names=['Y'])
            onx = ov.to_onnx(numpy.float32, numpy.bool_, verbose=0)
            self.assertNotIn("elem_type: 0", str(onx))
            oinf = OnnxInference(onx)
            x = numpy.array([-2, 2], dtype=numpy.float32)
            got = oinf.run({'X': x})
    ...
    def test_zif_onnx_common_intermediate(self):
        ...
        self.assertIn(
            "If(out_gre_0, else_branch=G1, then_branch=G2) -> Z", text)
    ...
    def test_zif_onnx_common_intermediate_level111(self):
        OnnxIf, OnnxIdentity = loadop("If", "Identity")

        yy = OnnxExisting(Variable('Y'))
        onx = OnnxIf(
            OnnxIdentity('X') == numpy.array([1], dtype=numpy.int64),
            output_names=['A']).then_do(yy + yy).else_do('Z')
        ...
        got = OnnxInference(model_def).run({'X': x, 'Y': y, 'Z': z})
        self.assertEqualArray(y + y, got['A'])
    ...
    def test_loop(self):
        OnnxLoop, OnnxAdd, OnnxIdentity = loadop("Loop", "Add", "Identity")

        m = numpy.array([5.5], dtype=numpy.float32)
        t = numpy.array([True])
