...
class TestOnnxrtValidateModelLabelEncoder(ExtTestCase):

    @ignore_warnings(category=(UserWarning, ConvergenceWarning, RuntimeWarning))
    def test_validate_sklearn_label_encoder(self):
        fLOG(__file__, self._testMethodName, OutputPrint=__name__ == "__main__")
        logger = getLogger('skl2onnx')
        logger.disabled = True
        verbose = 1 if __name__ == "__main__" else 0
        temp = get_temp_folder(__file__, "temp_validate_label_encoder")
        rows = list(enumerate_validated_operator_opsets(
            verbose, models={"LabelEncoder"},
        ...
        self.assertIn('LabelEncoder', set(piv['name']))
