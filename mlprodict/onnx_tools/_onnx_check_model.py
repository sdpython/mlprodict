...
"""
from onnx import TensorProto, TypeProto, ModelProto
...
def check_value_info(value_info, ctx):
    ...
    if value_case == "tensor_type":
        enforce_has_field(tt, "elem_type")
        enforce_has_field(tt, "shape")
    elif value_case == TypeProto.kOptionalType:
        type = value_info.type.optional_type()
        enforce_has_field(tt, "elem_type")
    elif value_case == TypeProto.kSequenceType:
        type = value_info.type.sequence_type()
        enforce_has_field(tt, "elem_type")
    elif value_case == TypeProto.kMapType:
        type = value_info.type.map_type()
        enforce_has_field(tt, "key_type")
        enforce_has_field(tt, "value_type")
    elif value_case == TypeProto.kOpaqueType:
        pass
    elif value_case == TypeProto.kSparseTensorType:
        type = value_info.type.sparse_tensor_type()
        enforce_has_field(tt, "elem_type")
        enforce_has_field(tt, "shape")
    else:
        raise OnnxCheckError(
...
def check_tensor(tensor, ctx):
    ...
    def check_data_field(field):
        has = getattr(tensor, field)().size()
        if has:
            num_value_fields += 1
    ...
    value_field = (
        check_data_field(float_data) or
        check_data_field(int32_data) or
        check_data_field(string_data) or
        check_data_field(int64_data) or
        check_data_field(raw_data) or
        check_data_field(double_data) or
        check_data_field(uint64_data))

    stored_externally = tensor.has_data_location() and tensor.data_location() == TensorProto.EXTERNAL
    ...
    if has_raw_data:
        ...
        def check_field(field):
            if nelem != 0 and getattr(tensor, field)().size():
                raise OnnxCheckError(
...
def check_sequence(sequence, ctx):
...
def check_optional(optional, ctx):
...
def check_sparse_tensor_indices_1(indices, sparse_tensor_proto, nnz):
    """
    ...
    # non-zero value.
    index_data = ParseData(int64_t, indices)
...
def check_sparse_tensor_indices_2(indices, sparse_tensor_proto, nnz):
    """
    Check that the index data stored in a SparseTensorProto is valid.
...
def check_sparse_tensor(sparse_tensor_proto, ctx):
...
def check_attribute(attr, ctx, lex_ctx):
    ...
    def check_type(expected_type):
        if attr.has_type() and attr.type != expected_type:
            raise OnnxCheckError(
                f"Type field and data field mismatch in attribute '{attr.name}'.",
                attr)

    def check_singular_field(field, type):
        if getattr(attr, 'has_' + field)():
            used_fields += 1
            check_type(type)
...
def check_node(node, ctx, lex_ctx):
    ...
    dit = opset_imports.find(node.domain)
    ...
    if not schema:
        if (node.domain == ONNX_DOMAIN or node.domain == AI_ONNX_ML_DOMAIN or
                node.domain == "ai.onnx" or node.domain == AI_ONNX_TRAINING_DOMAIN):
...
def check_graph(graph, ctx, parent_lex):
...
def get_version_for_domain(domain, opset_imports):
...
def check_opset_compatibility(node, ctx, func_opset_imports, model_opset_imports):
    func_opset_version = get_version_for_domain(node.domain, func_opset_imports)
    model_opset_version = get_version_for_domain(node.domain, model_opset_imports)
...
def check_model_local_functions(model, ctx, parent_lex):
...
def check_function(function, ctx, parent_lex):
...
def check_model(model, full_check=True):
    ctx = CheckerContext()
