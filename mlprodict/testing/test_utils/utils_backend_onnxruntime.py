...
from .utils_backend_common import (
    ...
    OnnxBackendAssertionError as OnnxRuntimeAssertionError,
    OnnxBackendMissingNewOnnxOperatorException as OnnxRuntimeMissingNewOnnxOperatorException,
    _compare_expected,
    _create_column,
)
...
def compare_runtime(test, decimal=5, options=None,  # pylint: disable=R0912
    ...
    try:
        ...
        if intermediate_steps:
            raise NotImplementedError("Cannot see intermediate steps.")
        if "-Fail" in onx:
            raise ExpectedAssertionError(
