...
def rename_in_onnx_graph(graph: GraphProto, replacements: Dict[str, str]
    ...
    for node in graph.node:
        if len(set(node.input) & set_rep) == 0:
            ...
            new_node = make_node(node.op_type, new_inputs, node.output,
                                 domain=node.domain)
...
def onnx_convert_model_for_opsets(model: ModelProto,
    ...
    for domain, version in target_opsets.items():
        ...
        if existing_opsets[domain] == target_opsets[domain]:
            continue
        domains.append((domain, existing_opsets.get(domain, None),
                        target_opsets.get(domain, None)))
    if len(domains) == 1 and domains[0][0] == "":
        # Use the conversion.
        new_model = convert_version(model, domains[0][2])
    elif len(domains) > 1:
        msg = ", ".join(f"domain={d!r}, from {before} -> {after}"
                        for b, before, after in domains)
        raise RuntimeError(
