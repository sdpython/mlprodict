    ...
    for (size_t dim = 0; dim < rank; ++dim) {
        if (dim >= strides_p.size() || dim >= kernel_shape.size() ||
            ...
            throw std::invalid_argument(MakeString(
                ...
                dim, ", strides.size()=", strides_p.size(), ", kernel_shape.size()=",
        ...
        if (dim_size <= 0)
            throw std::invalid_argument(MakeString(
                "Invalid argument in infer_output_shape, ComputePadAndOutputShape returned dim_size=",
                dim_size, "."));
...
void ConvPoolCommonShape::infer_output_shape(
    ...
    ::infer_output_shape(input_shape, kernel_shape, strides_p, dilations_p,
