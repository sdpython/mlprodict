...
public:
	...
	py_array_t<T> compute(
		py_array_t<T, py_array_style> X,
		float x_scale, T x_zero_point,
		py_array_t<T, py_array_style> W,
		py_array_t<float, py_array_style> w_scale,
		T w_zero_point,
		float y_scale, T y_zero_point,
		py_array_t<T, py_array_style> B) const {
...
private:
	...
	void compute_gil_free(
		py_array_t<T> X, py_array_t<T> W,
		py_array_t<T> B, py_array_t<T>& Y,
		...
		float x_scale, T x_zero_point,
		py_array_t<float> w_scale, T w_zero_point,
		float y_scale, T y_zero_point,
		...
		T* transpose_input_buffer = nullptr;
		T* transpose_output_buffer = nullptr;
		...
		T_const_ptr* col_buffer = nullptr;
		T_const_ptr* col_data = nullptr;
		...
		if (is_depthwise_conv) {
			...
			col_data = new T_const_ptr[kernel_size * output_image_size];
			col_buffer = col_data;
			...
			col_data = new T_const_ptr[group_col_buffer_size];
			col_buffer = col_data;
		}
			...
			if (!is_depthwise_conv && col_buffer && kernel_rank > 2) {
				for (int64_t group_id = 0; group_id < group_count; ++group_id) {
					Im2col_NCHW<T>(
						...
						((T*)col_buffer) + group_id * col_buffer_size,  // sizeof(T*) > sizeof(T)
						x_zero_point);
				}
			}
...
#pragma omp parallel for
			...
			for (int32_t batch_idx = 0; batch_idx < thread_count; ++batch_idx) {
				...
				if (is_depthwise_conv) {
					T_const_ptr* worker_col_buffer = col_buffer + output_start * kernel_size;
					Im2col_NHWC<T>(
						input_data,
						C,
						input_shape.data(),
					...
					for (int64_t group_id = 0; group_id < group_count; ++group_id) {
						...
						if (col_buffer) {
							T* worker_col_buffer = ((T*)col_buffer) + output_start * kernel_dim;
							if (kernel_rank == 2) {
								Im2col_NHWC<T>(
									input_data + group_id * group_input_channels,
									group_input_channels,
