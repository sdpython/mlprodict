...
def numpy_matmul_inplace(inplaces, a, b):
    ...
    if inplaces.get(1, False) and hasattr(b, 'flags'):
        return _numpy_matmul_inplace_right(a, b)
    return numpy.matmul(a, b)


def _numpy_matmul_inplace_left(a, b):
    "Subpart of @see fn numpy_matmul_inplace."
    if a.flags['F_CONTIGUOUS']:
        if len(b.shape) == len(a.shape) == 2 and b.shape[1] <= a.shape[1]:
            try:
                numpy.matmul(a, b, out=a[:, :b.shape[1]])
                return a[:, :b.shape[1]]
            except ValueError:
                return numpy.matmul(a, b)
        if len(b.shape) == 1:
            try:
                numpy.matmul(a, b.reshape(b.shape[0], 1), out=a[:, :1])
                return a[:, :1].reshape(a.shape[0])
            except ValueError:  # pragma no cover
                return numpy.matmul(a, b)
    return numpy.matmul(a, b)
...
def _numpy_matmul_inplace_right(a, b):
    "Subpart of @see fn numpy_matmul_inplace."
    if b.flags['C_CONTIGUOUS']:
        if len(b.shape) == len(a.shape) == 2 and a.shape[0] <= b.shape[0]:
            try:
                numpy.matmul(a, b, out=b[:a.shape[0], :])
                return b[:a.shape[0], :]
            except ValueError:  # pragma no cover
                return numpy.matmul(a, b)
        if len(a.shape) == 1:
            try:
                numpy.matmul(a, b, out=b[:1, :])
                return b[:1, :]
            except ValueError:  # pragma no cover
                return numpy.matmul(a, b)
    return numpy.matmul(a, b)
